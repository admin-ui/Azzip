...
var popUpCal = {
	...
	currentTimeText: '现在', // Display text for current month link
	...
	selectDate: function(Input) {
		...
		var timeSel = this.input.data("calendar.showTime") == "yes" ? this.timeSeparators : [];
		if (timeSel && timeSel.length > 1) Input.val(this.formatDate(this.selectedDay, this.selectedMonth, this.selectedYear, this.selectedHour, this.selectedMinute));
	...
	showCalendar: function() {
		...
		var timeSelect = '', timeSel;
		timeSel = this.input.data("calendar.showTime") == "yes" ? this.timeSeparators : [];

		if (timeSel && timeSel.length > 1) {
			...
			timeSelect += '</select>' + timeSel[1] + '<select id="calendar_minute">';
			for (var a = 0; a < 59; a++) {
			...
			if (timeSel.length == 4) {
				...
				timeSelect += '<option value="AM"' + ((this.selectedHour < 12) ? ' selected="selected"' : '') + '>' + timeSel[2] + '</option>';
				timeSelect += '<option value="PM"' + ((this.selectedHour >= 12) ? ' selected="selected"' : '') + '>' + timeSel[3] + '</option>';
				timeSelect += '</select>';
			}
		}

		var tct = (timeSel && timeSel.length > 1 ? this.currentTimeText : this.currentText);
		var html = '<div id="calendar_control">' +
			'<a id="calendar_prev">' + this.prevText + '</a>' +
			'<a id="calendar_next">' + this.nextText + '</a>' +
			'<a id="calendar_current">' + tct + '</a>' +
			'<a id="calendar_close">' + this.closeText + '</a>' + 
			'<a id="calendar_clear">' + this.clearText + '</a>' +
			'</div>';

            html += '<table id="calendar" cellpadding="0" cellspacing="0"><thead>' +
		...
		html += '</tbody></table><!--[if lte IE 6.5]><iframe src="javascript:false;" id="calendar_cover"></iframe><![endif]-->'             + (this.closeAtTop ? '' : '<div id="calendar_control"><a id="calendar_clear">' + this.clearText + '</a>' 
            + '<a id="calendar_close">' + this.closeText + '</a></div>');
            
        html+='<div id="calendar_header">';
		if (!this.changeYear) {
	...
	showFor: function(target) {
		...
		popUpCal.input._showTime = this.showTimeSelect;
	...
	setupActions: function() {
		...
		$('#calendar_current').click(function() { // back to today
			...
			popUpCal.adjustDate(); 
			var timeSel = popUpCal.input.data("calendar.showTime") == "yes" ? popUpCal.timeSeparators : [];
			if (timeSel && timeSel.length > 1) popUpCal.input.val(popUpCal.formatDate(popUpCal.selectedDay, popUpCal.selectedMonth, popUpCal.selectedYear, popUpCal.selectedHour, popUpCal.selectedMinute));
			else popUpCal.input.val(popUpCal.formatDate(popUpCal.selectedDay, popUpCal.selectedMonth, popUpCal.selectedYear));
            if(!popUpCal.ie6)popUpCal.hideCalendar(popUpCal.speed);
		});
	...
	setPos: function(targetObj, moveObj) {
		var coords = this.findPos(targetObj), p = popUpCal.scrollPos();
		// moveObj.css('position', 'absolute')
		// 	.css({position: "absolute", visibility: "visible", display:"block"});
		// console.log($(targetObj).height(), moveObj.height());
		moveObj.css('position', 'absolute').css('left', coords[0] + 'px').
			css('top', (coords[1] + targetObj.offsetHeight - 1) + 'px');
	},
...
$.fn.calendar = function(settings) {
	var showTime = false;
	if (settings && settings.showTimeSelect) {
		showTime = settings.showTimeSelect;
		delete settings.showTimeSelect;
	}
	...
	return this.each(function() {
		if (this.nodeName.toLowerCase() == 'input') {
			$(this).data("calendar.showTime", showTime ? "yes" : "no");
